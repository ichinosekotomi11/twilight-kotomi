...
func (a *App) runSchedulerJob(r *http.Request, jobID string) (map[string]any, []string, error) {
	...
	case "check_expired":
		if a.cfg().RetentionEnabled {
			return a.runRetentionExpiryJob(r)
		}
		disabled := 0
		embyDisabled := 0
		skippedProtected := 0
	...
	case "telegram_media_rank_daily", "telegram_media_rank_weekly", "telegram_media_rank_monthly":
		...
		return map[string]any{"success": true, "period": period, "start_at": window.Start.Unix(), "end_at": window.End.Unix(), "series_count": len(seriesTop), "movie_count": len(movieTop)}, []string{"telegram media rank pushed"}, nil
	case "telegram_user_minutes_daily", "telegram_user_minutes_weekly", "telegram_user_minutes_monthly":
		if !a.telegramStatsPushEnabled() {
			return map[string]any{"success": true, "enabled": false, "reason": "telegram stats push disabled"}, []string{"telegram stats push disabled"}, nil
		}
		period := embyStatsPeriodDaily
		...
		return map[string]any{"success": true, "period": period, "start_at": window.Start.Unix(), "end_at": window.End.Unix(), "count": len(items)}, []string{"telegram user minutes rank pushed"}, nil
