...
func TestRegisterDoesNotSetWebAccountThirtyDayExpiryForChineseUsername(t *testing.T) {
	...
	user, ok := app.store().FindUserByUsername("测试用户")
	if !ok {
...
func TestCalculateMonthlyPlaybackFromActivity(t *testing.T) {
	loc := time.FixedZone("CST", 8*3600)
	monthStart := time.Date(2026, time.June, 1, 0, 0, 0, 0, loc)
	entries := []embyActivityLogEntry{
		{Type: "playback.stop", Name: "客厅电视 上 alice 已停止播放 第 2 集", UserID: "101", ItemID: "ep2", Date: "2026-06-26T13:41:56.906Z"},
		{Type: "playback.start", Name: "alice 在 客厅电视 上开始播放 第 2 集", UserID: "101", ItemID: "ep2", Date: "2026-06-26T13:11:56.906Z"},
		{Type: "playback.stop", Name: "客厅电视 上 alice 已停止播放 第 1 集", UserID: "101", ItemID: "ep1", Date: "2026-06-01T00:20:00+08:00"},
		{Type: "playback.start", Name: "alice 在 客厅电视 上开始播放 第 1 集", UserID: "101", ItemID: "ep1", Date: "2026-05-31T23:50:00+08:00"},
		{Type: "playback.stop", Name: "iPhone 上 bob 已停止播放 电影", UserID: "202", ItemID: "movie", Date: "2026-06-15T08:00:00Z"},
		{Type: "playback.start", Name: "bob 在 iPhone 上开始播放 电影", UserID: "202", ItemID: "movie", Date: "2026-06-15T06:45:00Z"},
		{Type: "user.authenticated", Name: "alice 已在 电视台 上被验证", UserID: "101", Date: "2026-06-26T13:42:40.877Z"},
	}

	totals := calculateMonthlyPlaybackFromActivity(entries, monthStart, loc)
	if got, want := totals["alice"], int64(50*60); got != want {
		t.Fatalf("alice playback=%d want=%d", got, want)
	}
	if got, want := totals["bob"], int64(75*60); got != want {
		t.Fatalf("bob playback=%d want=%d", got, want)
	}

	summary := embyMonthlyPlayback{Seconds: totals["alice"], Minutes: (totals["alice"] + 59) / 60, Label: formatPlaybackDuration(totals["alice"])}
	if summary.Minutes != 50 {
		t.Fatalf("monthly playback minutes=%d want=50", summary.Minutes)
	}
	if summary.Label != "50分钟" {
		t.Fatalf("monthly playback label=%q want %q", summary.Label, "50分钟")
	}
}

func TestEmbyPlaybackCycleStart(t *testing.T) {
	app := newTestApp(t)
	loc := time.FixedZone("CST", 8*3600)
	boundAt := time.Date(2026, time.June, 10, 12, 0, 0, 0, loc)
	now := time.Date(2026, time.July, 28, 15, 30, 0, 0, loc)
	user := store.User{EmbyID: "emby-1", EmbyBoundAt: boundAt.Unix()}

	got := app.embyPlaybackCycle(user, now, 30).Start
	want := time.Date(2026, time.July, 10, 12, 0, 0, 0, loc)
	if !got.Equal(want) {
		t.Fatalf("cycle start=%s want=%s", got.Format(time.RFC3339), want.Format(time.RFC3339))
	}
}
...
func TestFormatPlaybackDuration(t *testing.T) {
	...
	for _, tc := range cases {
		if got := formatPlaybackDuration(tc.seconds); got != tc.want {
			t.Fatalf("formatPlaybackDuration(%d)=%q want %q", tc.seconds, got, tc.want)
		}
	}
}
...
func TestDirectEmbyRegistrationRecordsRegcodeEquivalentGrant(t *testing.T) {
	...
	if updated.Role != store.RoleNormal {
		t.Fatalf("direct register should keep normal role, got %#v", updated)
	}
...
func TestSelfBindEmbyForcesThirtyDayNormalAccount(t *testing.T) {
	app := newTestApp(t)
	app.cfg().EmbyToken = "test-token"
	emby := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		...
		case r.Method == http.MethodPost && r.URL.Path == "/Users/AuthenticateByName":
			_, _ = w.Write([]byte(`{"User":{"Id":"bound-emby","Name":"bound-emby","Policy":{}}}`))
		case r.Method == http.MethodGet && r.URL.Path == "/Users/bound-emby":
			_, _ = w.Write([]byte(`{"Id":"bound-emby","Name":"bound-emby","Policy":{}}`))
		case r.Method == http.MethodPost && r.URL.Path == "/Users/bound-emby/Policy":
			_, _ = w.Write([]byte(`{}`))
		default:
			http.NotFound(w, r)
		}
	...
	user, err := app.store().CreateUser(store.User{Username: "bind-user", Role: store.RoleNormal, Active: true, ExpiredAt: time.Now().AddDate(0, 0, 5).Unix()})
	if err != nil {
		t.Fatal(err)
	}
	req := httptest.NewRequest(http.MethodPost, "/api/v1/users/me/emby/bind", strings.NewReader(`{"emby_username":"bound-emby","emby_password":"Strong123"}`))
	req = req.WithContext(context.WithValue(req.Context(), principalKey, principal{User: user}))
	rr := httptest.NewRecorder()
	app.handleBindEmby(rr, req, nil)
	if rr.Code != http.StatusOK {
		t.Fatalf("bind emby status=%d body=%s", rr.Code, rr.Body.String())
	}
	updated, _ := app.store().User(user.UID)
	if updated.EmbyID != "bound-emby" || updated.Role != store.RoleNormal {
		t.Fatalf("bind emby should persist linked normal user: %#v", updated)
	}
	wantExpiry := time.Now().AddDate(0, 0, 30).Unix()
	if updated.ExpiredAt < wantExpiry-120 || updated.ExpiredAt > wantExpiry+120 {
		t.Fatalf("bind emby should grant 30-day expiry, got %d want ~%d", updated.ExpiredAt, wantExpiry)
	}
}
...
func TestPendingEmbyUserCannotReplaceEntitlementWithRegisterCode(t *testing.T) {
	app := newTestApp(t)
