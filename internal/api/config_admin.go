...
func configSectionDefs() []configSectionDef {
	...
	return []configSectionDef{
		{Key: "Global", Title: "全局", Description: "基础运行参数", Category: "runtime", Collapsed: true, Fields: []configFieldDef{
			...
			{Key: "public_ip", Label: "公网 IP / 地址", Type: "string", Description: "当前站点对外访问使用的 IP 或域名，便于后续手动维护"},
			{Key: "panel_public_url", Label: "面板公网地址", Type: "string", Description: "Telegram 主页按钮打开的面板地址，例如 http://example.com:8000 或 https://panel.example.com"},
		...
		{Key: "TelegramStats", Title: "Telegram 榜单推送", Description: "Emby 播放榜单和用户时长榜单的定时推送配置", Category: "integration", Collapsed: true, Fields: []configFieldDef{
			{Key: "enabled", Label: "启用榜单推送", Type: "bool", Description: "总开关：关闭后所有 Telegram 榜单推送任务只保留接口统计，不发送消息"},
			{Key: "timezone", Label: "统计时区", Type: "string", Description: "排行榜时间窗口使用的时区，默认 Asia/Shanghai"},
			{Key: "push_chat_id", Label: "推送目标 Chat ID", Type: "string", Description: "优先推送到此 Telegram 用户 / 群组 / 频道 ID；留空时回退到第一个管理员 ID"},
			{Key: "top_limit", Label: "榜单数量", Type: "int", Description: "每份榜单显示前多少名，默认 10"},
			{Key: "media_rank_enabled", Label: "启用媒体榜单", Type: "bool", Description: "控制电视剧/电影播放榜单推送总开关"},
			{Key: "media_rank_daily_enabled", Label: "媒体日榜", Type: "bool", Description: "每天推送当日电视剧/电影播放榜"},
			{Key: "media_rank_weekly_enabled", Label: "媒体周榜", Type: "bool", Description: "每周日推送本周电视剧/电影播放榜"},
			{Key: "media_rank_monthly_enabled", Label: "媒体月榜", Type: "bool", Description: "每月最后一天推送本月电视剧/电影播放榜"},
			{Key: "media_rank_daily_time", Label: "媒体日榜时间", Type: "string", Description: "每日 HH:MM"},
			{Key: "media_rank_weekly_time", Label: "媒体周榜时间", Type: "string", Description: "每日 HH:MM；仅周日实际发送"},
			{Key: "media_rank_monthly_time", Label: "媒体月榜时间", Type: "string", Description: "每日 HH:MM；仅每月最后一天实际发送"},
			{Key: "user_minutes_rank_enabled", Label: "启用用户时长榜", Type: "bool", Description: "控制用户播放分钟排行榜推送总开关"},
			{Key: "user_minutes_rank_daily_enabled", Label: "用户日榜", Type: "bool", Description: "每天推送当天用户播放分钟排行榜"},
			{Key: "user_minutes_rank_weekly_enabled", Label: "用户周榜", Type: "bool", Description: "每周日推送本周用户播放分钟排行榜"},
			{Key: "user_minutes_rank_monthly_enabled", Label: "用户月榜", Type: "bool", Description: "每月最后一天推送本月用户播放分钟排行榜"},
			{Key: "user_minutes_rank_daily_time", Label: "用户日榜时间", Type: "string", Description: "每日 HH:MM"},
			{Key: "user_minutes_rank_weekly_time", Label: "用户周榜时间", Type: "string", Description: "每日 HH:MM；仅周日实际发送"},
			{Key: "user_minutes_rank_monthly_time", Label: "用户月榜时间", Type: "string", Description: "每日 HH:MM；仅每月最后一天实际发送"},
		}},
		{Key: "SAR", Title: "注册/邀请", Description: "注册、卡码、邀请关系和求片\n推荐在「注册码管理」和「邀请系统管理」页面操作", Category: "policy", Collapsed: true, Fields: []configFieldDef{
			...
			{Key: "manual_renew_code_enabled", Label: "允许手动续期码", Type: "bool", Description: "允许用户手动兑换 type=2 续期码"},
			{Key: "initial_expiry_days", Label: "新绑 Emby 初始天数", Type: "int", Description: "普通用户首次注册或绑定 Emby 后写入的初始有效天数"},
			{Key: "retention_enabled", Label: "启用自动保号", Type: "bool", Description: "普通 Emby 账户到期后按播放活跃度和积分自动续期或清理"},
			{Key: "retention_activity_days", Label: "活跃检查天数", Type: "int", Description: "到期时检查最近多少天的 Emby 播放活动"},
			{Key: "retention_required_minutes", Label: "活跃观看分钟", Type: "int", Description: "最近活跃检查窗口内累计至少观看多少分钟才算有效；0 表示只要有播放记录即可"},
			{Key: "retention_grace_days", Label: "保号宽限天数", Type: "int", Description: "有播放记录但积分不足时冻结 Emby 并等待的天数"},
			{Key: "registration_code_exchange_enabled", Label: "允许积分兑换注册码", Type: "bool", Description: "允许用户使用签到积分兑换一个 Emby 注册码"},
			{Key: "registration_code_exchange_cost", Label: "注册码兑换积分", Type: "int", Description: "兑换一个注册码需要消耗的积分"},
			{Key: "registration_code_exchange_days", Label: "兑换注册码天数", Type: "int", Description: "积分兑换的注册码可开通 Emby 的初始天数"},
		}},
		...
		{Key: "MoviePilot", Title: "MoviePilot v2", Description: "仅管理员和白名单用户可使用的媒体搜索与下载服务", Category: "integration", Collapsed: true, Fields: []configFieldDef{
			{Key: "enabled", Label: "启用 MoviePilot", Type: "bool", Description: "开启白名单专属 MoviePilot 接口"},
			{Key: "url", Label: "服务地址", Type: "string", Description: "MoviePilot v2 地址，如 http://127.0.0.1:3001"},
			{Key: "username", Label: "用户名", Type: "string", Description: "MoviePilot 登录用户名"},
			{Key: "password", Label: "密码", Type: "secret", Description: "MoviePilot 登录密码"},
			{Key: "access_token", Label: "Access Token", Type: "secret", Description: "可选；填写后优先使用 Token"},
			{Key: "download_points_enabled", Label: "下载扣除积分", Type: "bool", Description: "按资源体积扣除签到积分，仅对白名单普通功能生效"},
			{Key: "download_points_per_gb", Label: "每 GB 扣分", Type: "int", Description: "每下载 1 GB 资源消耗多少积分；向上取整"},
		}},
...
func configValues(cfg config.Config) map[string]map[string]any {
	return map[string]map[string]any{
		"Global": {
			"server_name": cfg.AppName, "server_icon": cfg.ServerIcon, "auth_background_url": cfg.AuthBackgroundURL, "public_ip": cfg.PublicIP, "panel_public_url": cfg.PanelPublicURL, "databases_dir": cfg.DatabaseDir, "redis_url": cfg.RedisURL, "telegram_mode": cfg.TelegramMode, "force_bind_telegram": cfg.ForceBindTelegram,
		...
		"TelegramStats": {
			"enabled": cfg.TelegramStatsEnabled, "timezone": cfg.TelegramStatsTimezone, "push_chat_id": cfg.TelegramStatsPushChatID, "top_limit": cfg.TelegramStatsTopLimit,
			"media_rank_enabled": cfg.TelegramStatsMediaRankEnabled, "media_rank_daily_enabled": cfg.TelegramStatsMediaRankDailyEnabled, "media_rank_weekly_enabled": cfg.TelegramStatsMediaRankWeeklyEnabled, "media_rank_monthly_enabled": cfg.TelegramStatsMediaRankMonthlyEnabled,
			"media_rank_daily_time": cfg.TelegramStatsMediaRankDailyTime, "media_rank_weekly_time": cfg.TelegramStatsMediaRankWeeklyTime, "media_rank_monthly_time": cfg.TelegramStatsMediaRankMonthlyTime,
			"user_minutes_rank_enabled": cfg.TelegramStatsUserMinutesRankEnabled, "user_minutes_rank_daily_enabled": cfg.TelegramStatsUserMinutesRankDailyEnabled, "user_minutes_rank_weekly_enabled": cfg.TelegramStatsUserMinutesRankWeeklyEnabled, "user_minutes_rank_monthly_enabled": cfg.TelegramStatsUserMinutesRankMonthlyEnabled,
			"user_minutes_rank_daily_time": cfg.TelegramStatsUserMinutesRankDailyTime, "user_minutes_rank_weekly_time": cfg.TelegramStatsUserMinutesRankWeeklyTime, "user_minutes_rank_monthly_time": cfg.TelegramStatsUserMinutesRankMonthlyTime,
		},
		"SAR": {
			...
			"manual_renew_code_enabled": !cfg.ManualRenewCodeDisabled, "initial_expiry_days": cfg.InitialExpiryDays, "retention_enabled": cfg.RetentionEnabled, "retention_activity_days": cfg.RetentionActivityDays,
			"retention_required_minutes": cfg.RetentionRequiredMinutes,
			"retention_grace_days":       cfg.RetentionGraceDays, "registration_code_exchange_enabled": cfg.RegistrationCodeExchangeEnabled,
			"registration_code_exchange_cost": cfg.RegistrationCodeExchangeCost, "registration_code_exchange_days": cfg.RegistrationCodeExchangeDays,
		},
		...
		"MoviePilot":   {"enabled": cfg.MoviePilotEnabled, "url": cfg.MoviePilotURL, "username": cfg.MoviePilotUsername, "password": cfg.MoviePilotPassword, "access_token": cfg.MoviePilotAccessToken, "download_points_enabled": cfg.MoviePilotDownloadPointsEnabled, "download_points_per_gb": cfg.MoviePilotDownloadPointsPerGB},
