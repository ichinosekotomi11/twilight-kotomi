...
func (a *App) handleTelegramUpdate(ctx context.Context, update map[string]any) {
	...
	if privateChat && (text == "控制台" || text == "主页") {
		a.telegramHandleDashboardShortcut(ctx, chatID, fromID)
		return
	}
	...
	case "/start", "/help", "/twihelp":
		...
		if command == "/start" {
			a.telegramSendStart(ctx, chatID, fromID)
			return
		}
...
func (a *App) confirmBindCodeInline(ctx context.Context, chatID int64, bind store.BindCode, code string, telegramID int64, username string) {
	...
	a.telegramSendBindSuccess(ctx, chatID, telegramID)
}
...
func (a *App) telegramHandleMe(ctx context.Context, chatID, telegramID int64) {
	...
	_ = a.telegramSendMessage(ctx, chatID, "当前 Telegram 尚未绑定 Kotomi 账号。")
}
...
func (a *App) telegramHandlePlayInfo(ctx context.Context, chatID, telegramID int64) {
	...
	if strings.TrimSpace(u.EmbyID) == "" {
		_ = a.telegramSendMessage(ctx, chatID, "请先绑定 Emby 账号后再查看播放信息。")
		return
	}
	now := time.Now()
	...
	lines := []string{
		...
		"",
		"账号: " + u.Username,
		"周期开始: " + monthly.Cycle.Start.Format("2006-01-02 15:04"),
		"周期结束: " + monthly.Cycle.End.Format("2006-01-02 15:04"),
		fmt.Sprintf("本周期已播放: %d 分钟", monthly.Minutes),
		fmt.Sprintf("剩余天数: %d 天", int(math.Ceil(monthly.Cycle.Remaining(now).Hours()/24))),
	}
	if len(records) > 0 {
...
func (a *App) telegramStartText() string {
	...
	title := firstNonEmpty(strings.TrimSpace(a.cfg().TelegramBotStartTitle), "Kotomi Bot")
...
func (a *App) telegramUserCommands() []telegramBotCommand {
	return []telegramBotCommand{
		{Command: "/start", Description: "打开用户面板"},
		{Command: "/bind", Description: "绑定 Telegram 到网站账号"},
		{Command: "/me", Description: "查看当前绑定状态"},
		{Command: "/emby", Description: "查看 Emby 状态"},
		{Command: "/playinfo", Description: "查看播放统计"},
		{Command: "/help", Description: "查看可用命令"},
	}
}

func (a *App) telegramAdminCommands() []telegramBotCommand {
	return []telegramBotCommand{
		{Command: "/admin", Description: "打开管理员面板"},
		{Command: "/stats", Description: "查看系统统计"},
		{Command: "/userinfo", Description: "查询指定用户"},
		{Command: "/twfind", Description: "搜索用户"},
		{Command: "/twishelp", Description: "查看管理员命令"},
	}
}

func (a *App) telegramSyncMenus(ctx context.Context) {
	if err := a.telegramSetMyCommands(ctx, a.telegramUserCommands(), map[string]any{"type": "all_private_chats"}); err != nil {
		zap.L().Warn("telegram sync user commands failed", zap.Error(err))
	}
	adminCommands := append(a.telegramUserCommands(), a.telegramAdminCommands()...)
	for _, adminID := range a.cfg().TelegramAdminIDs {
		if adminID <= 0 {
			continue
		}
		scope := map[string]any{"type": "chat", "chat_id": adminID}
		if err := a.telegramSetMyCommands(ctx, adminCommands, scope); err != nil {
			zap.L().Warn("telegram sync admin commands failed", zap.Int64("chat_id", adminID), zap.Error(err))
		}
	}
}

func (a *App) telegramSendStart(ctx context.Context, chatID, telegramID int64) {
	text := a.telegramStartText()
	if markup := a.telegramDashboardReplyMarkup(telegramID); markup != nil {
		_, _ = a.telegramSendMessageWithMarkup(ctx, chatID, text, markup)
		return
	}
	_ = a.telegramSendMessage(ctx, chatID, text)
}

func (a *App) telegramSendBindSuccess(ctx context.Context, chatID, telegramID int64) {
	text := "Telegram 绑定已确认，可以回到网页继续。"
	if markup := a.telegramDashboardReplyMarkup(telegramID); markup != nil {
		_, _ = a.telegramSendMessageWithMarkup(ctx, chatID, text, markup)
		return
	}
...
func (a *App) telegramDashboardReplyMarkup(telegramID int64) any {
	if telegramID <= 0 {
		return nil
	}
	if _, ok := a.store().FindUserByTelegramID(telegramID); !ok {
		return nil
	}
	panelURL := strings.TrimSpace(a.cfg().PanelPublicURL)
	if panelURL == "" {
		return nil
	}
	if strings.HasPrefix(strings.ToLower(panelURL), "https://") {
		return telegramReplyKeyboard([][]map[string]any{{
			{"text": "控制台", "web_app": map[string]string{"url": panelURL}},
		}})
	}
	return telegramReplyKeyboard([][]map[string]any{{
		{"text": "控制台"},
...
func (a *App) telegramHandleDashboardShortcut(ctx context.Context, chatID, telegramID int64) {
	u, ok := a.store().FindUserByTelegramID(telegramID)
	if !ok {
		_ = a.telegramSendMessage(ctx, chatID, "当前 Telegram 尚未绑定 Kotomi 账号。")
		return
	...
	panelURL := strings.TrimSpace(a.cfg().PanelPublicURL)
	if panelURL == "" {
		_ = a.telegramSendMessage(ctx, chatID, "管理员暂未配置面板公网地址。")
		return
	}
	text := "点击下方按钮进入主页。"
	if u.EmbyID == "" {
		text = "账号已绑定，点击下方按钮进入主页。"
	}
	_, _ = a.telegramSendMessageWithMarkup(ctx, chatID, text, telegramInlineURLKeyboard("进入主页", panelURL))
}
