...
// applyInitialEmbyEntitlement 把首次开通 Emby 的账号收敛到站点默认口径：
// 普通用户一律重置为 30 天；管理员 / 白名单保留原有身份与有效期，只清理
// retention 运行时标记，避免"新绑 Emby"后还继续处于过期缓冲态。
func initialExpiryDays(cfg config.Config) int {
	if cfg.InitialExpiryDays == 0 {
		return 30
	}
	return cfg.InitialExpiryDays
}

func effectiveEmbyBoundAt(u store.User) int64 {
	if strings.TrimSpace(u.EmbyID) == "" {
		return 0
	}
	if u.EmbyBoundAt > 0 {
		return u.EmbyBoundAt
	}
	return firstNonZeroInt64(u.RegisterTime, u.CreatedAt)
}

func embyPlaybackSince(u store.User, since time.Time) time.Time {
	boundAt := effectiveEmbyBoundAt(u)
	if boundAt <= 0 {
		return since
	}
	bound := time.Unix(boundAt, 0).In(since.Location())
...
func (a *App) applyInitialEmbyEntitlement(u *store.User, now time.Time) {
	if u.Role != store.RoleAdmin && u.Role != store.RoleWhitelist {
		u.Role = store.RoleNormal
		u.ExpiredAt = expiryFromDays(initialExpiryDays(*a.cfg()), now)
	}
...
func regcodeDTO(code store.RegCode) map[string]any {
	...
	// 创建时间 + 累计暂停秒数 + 有效小时数推算绝对过期时间戳（秒）。基准与 regcodeStatus 一致，
	...
	if code.ValidityTime > 0 {
		...
		expiresAt = base + code.PausedSeconds + code.ValidityTime*3600
	}
	...
	return map[string]any{
		...
		"paused_seconds":           code.PausedSeconds,
		"pause_start":              code.PauseStart,
		"note":                     code.Note,
