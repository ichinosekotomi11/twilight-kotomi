...
func telegramInlineURLKeyboard(text, rawURL string) any {
	return map[string]any{
...
func telegramReplyKeyboard(rows [][]map[string]any) any {
	return map[string]any{
		"keyboard":        rows,
...
func telegramCommandsPayload(commands []telegramBotCommand) []map[string]string {
	if len(commands) == 0 {
		return nil
	}
	...
	for _, command := range commands {
		...
		result = append(result, map[string]string{
			...
			"description": truncateString(description, 256),
		})
	}
	return result
}

func (a *App) telegramSetMyCommands(ctx context.Context, commands []telegramBotCommand, scope map[string]any) error {
	payload := telegramCommandsPayload(commands)
	if len(payload) == 0 {
		return nil
	}
	body := map[string]any{
	...
	return a.telegramPost(ctx, "setMyCommands", body, nil)
}
