...
func (a *App) handleBindEmby(w http.ResponseWriter, r *http.Request, _ Params) {
	...
	u, err = a.store().UpdateUser(u.UID, func(u *store.User) error {
		a.applyInitialEmbyEntitlement(u, time.Now())
		return nil
	})
	if statusFromError(w, err) {
		return
...
func (a *App) handleRenew(w http.ResponseWriter, r *http.Request, _ Params) {
	p := current(r)
	if a.cfg().ManualRenewCodeDisabled {
		failWithCode(w, http.StatusForbidden, ErrForbidden, "手动兑换续期码已关闭")
		return
	}
	if a.requireEmailVerified(w, p.User) {
		return
...
func (a *App) handleSystemInfo(w http.ResponseWriter, r *http.Request, _ Params) {
	...
	ok(w, "OK", map[string]any{
		...
		"features": map[string]any{
			...
			"moviepilot":                    cfg.MoviePilotEnabled,
...
func (a *App) handleEmbyStatus(w http.ResponseWriter, r *http.Request, _ Params) {
	...
	defer cancel()
	info, online := a.embyHealth(statusCtx)
	if info == nil {
		info = map[string]any{}
	}
	sessions := []map[string]any{}
	if online {
		sessionsCtx, sessionsCancel := context.WithTimeout(r.Context(), 2*time.Second)
		_ = a.embyGet(sessionsCtx, "/Sessions", &sessions)
		sessionsCancel()
	}
	u := current(r).User
	payload := map[string]any{"online": online, "server_name": firstNonEmpty(asString(info["ServerName"]), asString(info["Name"])), "version": firstNonEmpty(asString(info["Version"]), "unknown"), "operating_system": asString(info["OperatingSystem"]), "active_sessions": len(sessions), "total_sessions": len(sessions), "is_synced": u.EmbyID != "", "is_active": u.Active, "can_unbind": a.userCanSelfUnbindEmby(u), "message": "OK"}
	if online {
		countsCtx, countsCancel := context.WithTimeout(r.Context(), 2*time.Second)
		payload["library_counts"] = a.embyLibraryCounts(countsCtx)
		countsCancel()
	}
	if online && strings.TrimSpace(u.EmbyID) != "" {
		...
		payload["monthly_playback_minutes"] = playback.Minutes
		payload["monthly_playback_str"] = playback.Label
		payload["playback_cycle_start"] = playback.Cycle.Start.Unix()
		payload["playback_cycle_end"] = playback.Cycle.End.Unix()
		payload["playback_cycle_range"] = formatPlaybackCycleRange(playback.Cycle)
	}
	ok(w, "OK", payload)
}
...
func (a *App) embyLibraryCounts(ctx context.Context) map[string]int {
	...
	if err := a.embyGet(ctx, "/Items/Counts", &payload); err != nil {
		return map[string]int{"movies": 0, "series": 0, "episodes": 0}
	}
	return map[string]int{
		"movies":   int(numeric(payload["MovieCount"])),
		"series":   int(numeric(payload["SeriesCount"])),
		"episodes": int(numeric(payload["EpisodeCount"])),
	}
}

type embyMonthlyPlayback struct {
	Seconds int64
	Minutes int64
	Label   string
	Cycle   embyPlaybackCycle
}

type embyPlaybackCycle struct {
	Start time.Time
	End   time.Time
}

func (c embyPlaybackCycle) Remaining(now time.Time) time.Duration {
	if !now.Before(c.End) {
		return 0
	}
...
type embyActivityLogEntry struct {
...
func (a *App) embyMonthlyPlaybackSummary(ctx context.Context, user store.User, now time.Time) embyMonthlyPlayback {
	...
	return embyMonthlyPlayback{
		...
		Label:   formatPlaybackDuration(totalSeconds),
		Cycle:   cycle,
	}
}

func (a *App) embyPlaybackCycle(user store.User, now time.Time, cycleDays int) embyPlaybackCycle {
	if cycleDays <= 0 {
		cycleDays = 30
	}
	loc := a.embyStatsLocation()
	now = now.In(loc)
	boundAt := effectiveEmbyBoundAt(user)
	if boundAt <= 0 {
		return embyPlaybackCycle{
			Start: now,
			End:   now.Add(time.Duration(cycleDays) * 24 * time.Hour),
		}
	}
	bound := time.Unix(boundAt, 0).In(loc)
	cycle := time.Duration(cycleDays) * 24 * time.Hour
	if !bound.Before(now) {
		return embyPlaybackCycle{Start: bound, End: bound.Add(cycle)}
	}
	elapsed := now.Sub(bound)
	completedCycles := int64(elapsed / cycle)
	start := bound.Add(time.Duration(completedCycles) * cycle)
	return embyPlaybackCycle{Start: start, End: start.Add(cycle)}
}

func (a *App) embyMonthlyPlaybackTotals(ctx context.Context, now time.Time) map[string]int64 {
	monthKey := now.In(now.Location()).Format("2006-01")
	a.embyPlaybackMu.Lock()
	cached := a.embyPlaybackCache
	...
	totals := a.localMonthlyPlaybackTotals(monthStart.Unix())
	a.embyPlaybackMu.Lock()
	a.embyPlaybackCache = embyPlaybackCacheEntry{
		monthKey: monthKey,
		checked:  now,
...
func (a *App) embyPlaybackSecondsSince(ctx context.Context, embyUsername string, since, now time.Time) int64 {
	...
	total := int64(0)
	needle := normalizeActivityPlaybackUser(embyUsername)
...
func (a *App) localPlaybackSecondsForUser(uid int64, since int64, limit int) int64 {
	total := int64(0)
	for _, record := range a.store().PlaybackRecords(uid, since, limit) {
...
func (a *App) localMonthlyPlaybackTotals(since int64) map[string]int64 {
	totals := map[string]int64{}
	usersByUID := map[int64]store.User{}
	for _, user := range a.store().ListUsers() {
		usersByUID[user.UID] = user
	}
	for _, record := range a.store().PlaybackRecords(0, since, 10000) {
...
func calculateMonthlyPlaybackFromActivity(entries []embyActivityLogEntry, monthStart time.Time, loc *time.Location) map[string]int64 {
	...
	for _, entry := range entries {
		...
		if !strings.HasPrefix(entry.Type, "playback.") || userKey == "" || strings.TrimSpace(entry.ItemID) == "" {
			continue
		}
		...
		key := userKey + "|" + entry.ItemID
		switch entry.Type {
		case "playback.stop":
			if playedAt.Before(monthStart) {
				continue
			}
			pendingStops[key] = append(pendingStops[key], playedAt)
		case "playback.start":
			...
			startAt := playedAt
			if startAt.Before(monthStart) {
				startAt = monthStart
			}
			if stopAt.After(startAt) {
				totals[userKey] += int64(stopAt.Sub(startAt).Seconds())
			}
		}
	}
	for key, stops := range pendingStops {
		uid := key
		if idx := strings.IndexByte(key, '|'); idx >= 0 {
			uid = key[:idx]
		}
		for _, stopAt := range stops {
			if stopAt.After(monthStart) && stopAt.Sub(monthStart) <= 12*time.Hour {
				totals[uid] += int64(stopAt.Sub(monthStart).Seconds())
			}
		}
	}
	return totals
...
func activityPlaybackUserKey(entry embyActivityLogEntry) string {
	name := strings.TrimSpace(entry.Name)
	switch entry.Type {
	case "playback.start":
		if idx := strings.Index(name, " 在 "); idx > 0 {
			return normalizeActivityPlaybackUser(name[:idx])
		}
	case "playback.stop":
		prefixEnd := strings.Index(name, " 上 ")
		userEnd := strings.Index(name, " 已停止播放 ")
		if prefixEnd >= 0 && userEnd > prefixEnd+len(" 上 ") {
			return normalizeActivityPlaybackUser(name[prefixEnd+len(" 上 ") : userEnd])
		}
	}
	return normalizeActivityPlaybackUser(entry.UserID)
}

func normalizeActivityPlaybackUser(value string) string {
	return strings.TrimSpace(value)
}

func parseEmbyActivityTime(raw string, loc *time.Location) (time.Time, error) {
...
func formatPlaybackDuration(seconds int64) string {
	if seconds <= 0 {
		return "0分钟"
	}
...
func formatPlaybackCycleRange(cycle embyPlaybackCycle) string {
	if cycle.Start.IsZero() || cycle.End.IsZero() {
		return ""
	}
	return cycle.Start.Format("2006-01-02 15:04") + " ~ " + cycle.End.Format("2006-01-02 15:04")
}
...
func (a *App) handleAdminDeleteUser(w http.ResponseWriter, r *http.Request, params Params) {
	...
	for _, targetUID := range a.collectCascadeUIDs(uid, depth) {
		...
		if mode == "emby_only" {
			...
			_, err := a.store().UpdateUser(targetUID, func(u *store.User) error { u.EmbyID = ""; u.EmbyUsername = ""; u.EmbyBoundAt = 0; return nil })
			if err != nil {
...
func (a *App) handleAdminBindEmby(w http.ResponseWriter, r *http.Request, params Params) {
	...
	updatedUser, err := a.store().UpdateUser(updatedUser.UID, func(u *store.User) error {
		a.applyInitialEmbyEntitlement(u, time.Now())
		return nil
	})
	if statusFromError(w, err) {
