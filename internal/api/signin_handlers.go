...
func (a *App) handleSigninExchangeRegistrationCode(w http.ResponseWriter, r *http.Request, _ Params) {
	cfg := *a.cfg()
	if !cfg.SigninEnabled || !cfg.RegistrationCodeExchangeEnabled {
		failWithCode(w, http.StatusForbidden, ErrForbidden, "积分兑换注册码未开启")
		return
	}
	p := current(r)
	if a.userIsProtected(p.User) {
		failWithCode(w, http.StatusForbidden, ErrForbidden, "管理员和白名单无需兑换普通注册码")
		return
	}
	cost := cfg.RegistrationCodeExchangeCost
	days := cfg.RegistrationCodeExchangeDays
	if cost <= 0 || days <= 0 {
		failWithCode(w, http.StatusConflict, ErrConflict, "注册码兑换配置无效")
		return
	}
	format := a.regCodeFormatForType(1, cfg.RegisterCodeFormat)
	algorithm := firstNonEmpty(cfg.RegCodeRandomAlgorithm, "base32-20")
	var code string
	var signin store.Signin
	var err error
	for attempt := 0; attempt < 20; attempt++ {
		candidate := generateRegCode(format, 1, algorithm, days, 1, -1, 1)
		signin, err = a.store().SpendSigninPointsAndCreateRegCode(p.User.UID, cost, store.RegCode{
			Code: candidate, Type: 1, Days: days, ValidityTime: -1, UseCountLimit: 1,
			Note: "用户积分兑换",
		})
		if err == nil {
			code = candidate
	...
	if err != nil {
		if errors.Is(err, store.ErrInsufficientPoints) {
			failWithCode(w, http.StatusConflict, ErrSigninInsufficientPoints, "积分不足，无法兑换注册码")
			return
		}
		statusFromError(w, err)
		return
	}
	a.audit(r, "exchange_registration_code", "user", 0, map[string]any{"cost": cost, "days": days})
	ok(w, "注册码兑换成功", map[string]any{
		"code": code, "cost": cost, "days": days,
		"currency_name": signinCurrencyName(cfg), "remaining_points": signin.Points,
	})
}
...
func signinConfigPayload(cfg config.Config) map[string]any {
	return map[string]any{
		...
		"registration_code_exchange": map[string]any{
			"enabled": cfg.SigninEnabled && cfg.RegistrationCodeExchangeEnabled,
			"cost":    cfg.RegistrationCodeExchangeCost, "days": cfg.RegistrationCodeExchangeDays,
		},
	}
}
...
func signinSummaryPayloadAt(cfg config.Config, si store.Signin, now time.Time) map[string]any {
	...
	return map[string]any{
		...
		"registration_code_exchange": map[string]any{
			"enabled": cfg.SigninEnabled && cfg.RegistrationCodeExchangeEnabled,
			"cost":    cfg.RegistrationCodeExchangeCost, "days": cfg.RegistrationCodeExchangeDays,
			"affordable": si.Points >= cfg.RegistrationCodeExchangeCost,
		},
	}
}
