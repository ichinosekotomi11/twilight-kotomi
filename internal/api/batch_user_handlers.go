...
func (a *App) handleBatchPromoteWhitelist(w http.ResponseWriter, r *http.Request, _ Params) {
	payload := decodeMap(r)
	if stringValue(payload, "confirm") != confirmBatchPromoteWL {
		failWithCode(w, http.StatusBadRequest, ErrBatchConfirmRequired, "missing confirm "+confirmBatchPromoteWL)
		return
	}
	uids, okPayload := a.batchUserUIDsFromPayload(w, payload, 200, 5000, "too many users in one batch")
	if !okPayload {
	...
	for _, uid := range uids {
		...
		if target.Role == store.RoleAdmin {
			addBatchOutcomeWithCode(result, uid, ErrUserProtected, fmt.Errorf("cannot promote admin account"))
			continue
		}
		if a.configuredAdminMatch(target.UID, target.Username) {
			addBatchOutcomeWithCode(result, uid, ErrUserProtected, fmt.Errorf("cannot promote configured admin account"))
			continue
		}
		if target.Role != store.RoleWhitelist {
			if _, err := a.store().SetUserRoleAtomic(uid, store.RoleWhitelist); err != nil {
				addBatchOutcome(result, uid, err)
				continue
			}
		}
		...
		if err == nil && strings.TrimSpace(updated.EmbyID) != "" {
			if syncErr := a.embyApplyEnabledState(r.Context(), updated.UID, updated.EmbyID, a.embyShouldEnableUser(updated)); syncErr != nil {
				err = syncErr
			}
		}
		addBatchOutcome(result, uid, err)
	...
	result["selected_all"] = boolValue(payload, "select_all", false)
	result["role"] = store.RoleWhitelist
	result["expired_at"] = -1
	a.audit(r, "batch_promote_whitelist", "admin", 0, map[string]any{
		"success": result["success"], "failed": result["failed"],
	})
	ok(w, "批量升级白名单完成", result)
}
