...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-start justify-between gap-4 overflow-hidden rounded-md border p-4 pr-9 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
...
>(({ className, variant, type, ...props }, ref) => {
  // 屏幕阅读器朗读优先级：
  //   destructive → foreground（Radix 内部映射为 role="alert" + aria-live="assertive"）
  //   其它 / success → background（role="status" + aria-live="polite"）
  // 业务侧仍可显式传 type 覆盖默认。
  const effectiveType = type ?? (variant === "destructive" ? "foreground" : "background");
  return (
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      // focus → focus-visible：仅键盘导航显示焦点环，鼠标点击不绘制
      // 。Radix 已正确分发 focus-visible 状态。
