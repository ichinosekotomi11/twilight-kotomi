...
export default function AdminIndexPage() {
  ...
  const visibleAdminPages = useMemo(() => {
    return adminPages.filter((page) => {
      if (systemInfo?.features?.media_request === false && page.href === "/admin/requests") {
        return false;
      }
      if (systemInfo?.features?.ticket_system === false && page.href === "/admin/tickets") {
        return false;
      }
      if (systemInfo?.features?.bangumi_sync === false && page.href === "/admin/bangumi") {
        return false;
      }
      return true;
    });
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-8">
      ...
      {Object.entries(categoryConfig).map(([catKey, config]) => {
        const pages = visibleAdminPages.filter((p) => p.category === catKey);
