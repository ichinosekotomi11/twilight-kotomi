...
export default function AdminUsersPage() {
  ...
  const handlePromoteWhitelist = async (user: UserInfo) => {
    const ok = await confirmAction({
      title: t("adminUsers.promoteWhitelistTitle" as any),
      description: t("adminUsers.promoteWhitelistDescription" as any, { username: user.username }),
      tone: "warning",
      confirmLabel: t("adminUsers.promoteWhitelistConfirmLabel" as any),
    });
    if (!ok) return;
    setIsActionLoading(true);
    try {
      const roleRes = await api.updateUser(user.uid, { role: 2, active: true });
      if (!roleRes.success) {
        toast({ title: "升级白名单失败", description: roleRes.message, variant: "destructive" });
        return;
      }
      const expiryRes = await api.adminSetUserExpiry(user.uid, { permanent: true });
      if (!expiryRes.success) {
        toast({ title: "升级白名单失败", description: expiryRes.message, variant: "destructive" });
        return;
      }
      toast({
        title: t("adminUsers.promoteWhitelistSuccessTitle" as any),
        description: t("adminUsers.promoteWhitelistSuccessDescription" as any, { username: user.username }),
        variant: "success",
      });
      invalidateUsersCache();
      await loadUsers();
    } catch (error: any) {
      toast({ title: "升级白名单失败", description: error.message, variant: "destructive" });
    } finally {
      setIsActionLoading(false);
    }
  };
  ...
  const handleSelectedPromoteWhitelist = async () => {
    if (selectedCount === 0) return;
    const confirmed = await confirmAction({
      title: "批量升级白名单？",
      description: `将把 ${selectedCount} 个已选用户升级为白名单，账号自动转为永久并启用。管理员会自动跳过。`,
      tone: "warning",
      confirmLabel: "升级白名单",
    });
    if (!confirmed) return;
    setBatchUserLoading(true);
    try {
      const res = await api.batchPromoteWhitelist(selectedBatchTarget());
      if (res.success && res.data) {
        toast({
          title: "批量升级白名单完成",
          description: `成功 ${res.data.success} 个，失败 ${res.data.failed} 个`,
        ...
        toast({ title: "批量升级白名单失败", description: res.message, variant: "destructive" });
      }
    } catch (error: any) {
      toast({ title: "批量升级白名单失败", description: error.message, variant: "destructive" });
    } finally {
      setBatchUserLoading(false);
    }
  };
  ...
  const renderUserActions = (user: UserInfo) => (
    <UserActionsMenu
      ...
      handlers={{
        ...
        onPromoteWhitelist: handlePromoteWhitelist,
          ...
          <CardContent className="flex flex-col gap-4 p-4 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex flex-wrap items-center gap-2 rounded-md border bg-muted/20 p-1">
                ...
                <Button variant="outline" size="sm" onClick={() => void handleSelectedPromoteWhitelist()} disabled={batchUserLoading || selectedCount === 0}>
