...
import { api, type CodeUsePreview, type EmbyInfo, type EmbyOnlineInfo, type EmbyTodayStats, type MediaRequest, type TelegramStatus, type SigninSummary, type RegisterAvailability, type EmbyRegisterStatus } from "@/lib/api";
...
export default function DashboardPage() {
  ...
  const [embyOnlineInfo, setEmbyOnlineInfo] = useState<EmbyOnlineInfo | null>(null);
  const [embyTodayStats, setEmbyTodayStats] = useState<EmbyTodayStats | null>(null);
  ...
  const hasBoundEmby = Boolean(user?.emby_id) && !user?.pending_emby;
  const canSeeOnlineUserDetails = user?.role === 0 || user?.role === 2;
  ...
  const loadDashboardData = useCallback(async (signal?: AbortSignal) => {
    ...
    const [tgSettled, embySettled, onlineSettled, linesSettled, todayStatsSettled, urlsSettled, reqSettled, signinSettled, registerSettled] = await Promise.allSettled([
      api.getTelegramStatus(),
      api.getEmbyInfo(),
      api.getEmbyOnline(),
      api.getEmbyLines(),
      api.getEmbyTodayStats(),
      api.getEmbyUrls(),
      myRequestsPromise,
    ...
    inspect("emby_online", onlineSettled);
    inspect("emby_lines", linesSettled);
    inspect("emby_today_stats", todayStatsSettled);
    ...
    if (onlineSettled.status === "fulfilled" && onlineSettled.value.success && onlineSettled.value.data) {
      setEmbyOnlineInfo(onlineSettled.value.data);
    } else {
      setEmbyOnlineInfo(null);
    }
    if (todayStatsSettled.status === "fulfilled" && todayStatsSettled.value.success && todayStatsSettled.value.data) {
      setEmbyTodayStats(todayStatsSettled.value.data);
    } else {
      setEmbyTodayStats(null);
    }
    if (linesSettled.status === "fulfilled" && linesSettled.value.success && linesSettled.value.data) {
      applyEmbyUrls(linesSettled.value.data);
    }
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6 pb-10">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <motion.div variants={item} className="premium-card p-5 sm:p-6 flex flex-col gap-3">
          ...
          <div className="min-w-0 space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground break-all">
              {t("dashboard.embyCurrentExpiry")}
              {!user?.emby_id ? t("dashboard.unbound") : isPendingEmby ? t("dashboard.notOpened") : (user.expire_status || "--")}
            </p>
            {user?.emby_id && (
              <p className="text-xs text-muted-foreground break-all">
                {t("dashboard.embyMonthlyPlayback")}
                {embyInfo?.monthly_playback_str || t("dashboard.noPlaybackThisMonth")}
              </p>
            )}
            {user?.emby_id && embyInfo?.playback_cycle_range && (
              <p className="text-xs text-muted-foreground break-all">
                {t("dashboard.embyPlaybackCycleRange", { range: embyInfo.playback_cycle_range })}
              </p>
            )}
            {user?.emby_id ? (
            ...
            {user?.emby_id && embyInfo?.library_counts && (
              <div className="grid grid-cols-3 gap-2 pt-2">
                <div className="rounded-xl border border-border/60 bg-background/60 p-2">
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t("dashboard.libraryMovies")}</p>
                  <p className="mt-1 text-lg font-black">{embyInfo.library_counts.movies ?? 0}</p>
                </div>
                <div className="rounded-xl border border-border/60 bg-background/60 p-2">
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t("dashboard.librarySeries")}</p>
                  <p className="mt-1 text-lg font-black">{embyInfo.library_counts.series ?? 0}</p>
                </div>
                <div className="rounded-xl border border-border/60 bg-background/60 p-2">
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t("dashboard.libraryEpisodes")}</p>
                  <p className="mt-1 text-lg font-black">{embyInfo.library_counts.episodes ?? 0}</p>
                </div>
              </div>
            )}
          </div>
        ...
        {hasBoundEmby && (
        <motion.div variants={item} className="premium-card p-5 sm:p-6 flex flex-col gap-4">
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              <div className="p-2 bg-violet-500/10 text-violet-500 rounded-xl">
                <Activity className="h-4 w-4" />
              </div>
              <h3 className="text-sm font-black uppercase tracking-widest text-muted-foreground">{t("dashboard.embyOnlineAndLines")}</h3>
            </div>
            <Badge variant="outline" className="text-[10px] shrink-0">
              {t("dashboard.currentOnlineCount", { count: embyOnlineInfo?.current_online ?? 0 })}
            </Badge>
          </div>
          ...
          <div className="space-y-2">
            <p className="text-sm font-bold">
              {embyOnlineInfo
                ? embyOnlineInfo.current_online > 0
                  ? t("dashboard.currentOnlineCount", { count: embyOnlineInfo.current_online })
                  : t("dashboard.noOneOnline")
                : isLoading
                  ? t("dashboard.loadingStats")
                  : t("dashboard.statsLoadFailed")}
            </p>
            {canSeeOnlineUserDetails && embyOnlineInfo && embyOnlineInfo.users.length > 0 && (
              <div className="space-y-1 text-xs text-muted-foreground">
                {embyOnlineInfo.users.map((onlineUser, index) => (
                  <div key={`${onlineUser.username}-${index}`} className="rounded-xl border border-border/60 bg-background/60 px-3 py-2">
                    <p className="font-semibold text-foreground">{onlineUser.username}</p>
                    <p className="truncate">{onlineUser.item_name || t("dashboard.playingUnknown")}</p>
                    <p className="truncate">{[onlineUser.client, onlineUser.device_name].filter(Boolean).join(" · ")}</p>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="space-y-2 min-w-0">
            <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t("dashboard.availableLines")}</p>
            {lineSlots.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                {linesRequireEmby ? t("dashboard.noLinesRequiresEmby") : t("dashboard.noLines")}
              </p>
            ) : (
              <div className="space-y-2">
                {lineSlots.map((slot) => (
                  <div key={`dashboard-${slot.key}`} className="rounded-xl border border-border/60 bg-background/60 px-3 py-2">
                    <p className="text-xs font-semibold text-foreground">{slot.name}</p>
                    <p className="mt-1 break-all text-xs text-muted-foreground">{slot.url}</p>
                  </div>
                ))}
              </div>
            )}
      ...
      <motion.div variants={item} className="premium-card p-5 sm:p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-base font-black tracking-tight">{t("dashboard.embyTodayStatsTitle")}</h3>
            <p className="text-[11px] text-muted-foreground font-bold uppercase tracking-tighter">{t("dashboard.embyTodayStatsSubtitle")}</p>
          </div>
        </div>

        {!embyTodayStats ? (
          <p className="text-sm text-muted-foreground">
            {isLoading ? t("dashboard.loadingStats") : t("dashboard.statsLoadFailed")}
          </p>
        ) : (
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-2">
                <h4 className="text-sm font-black tracking-tight">{t("dashboard.seriesTopToday")}</h4>
                <Badge variant="outline" className="text-[10px]">{t("dashboard.topCount", { count: embyTodayStats.series_top.length })}</Badge>
              </div>
              {embyTodayStats.series_top.length === 0 ? (
                <p className="text-sm text-muted-foreground">{t("dashboard.noSeriesStatsToday")}</p>
              ) : (
                <div className="space-y-2">
                  {embyTodayStats.series_top.map((entry, index) => (
                    <div key={entry.key || `${entry.title}-${index}`} className="rounded-xl border border-border/60 bg-background/60 px-4 py-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="truncate text-sm font-semibold">{index + 1}. {entry.title}</p>
                          <p className="mt-1 text-xs text-muted-foreground">{t("dashboard.rankSummary", { plays: entry.play_count, minutes: entry.total_minutes })}</p>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-2">
                <h4 className="text-sm font-black tracking-tight">{t("dashboard.movieTopToday")}</h4>
                <Badge variant="outline" className="text-[10px]">{t("dashboard.topCount", { count: embyTodayStats.movie_top.length })}</Badge>
              </div>
              {embyTodayStats.movie_top.length === 0 ? (
                <p className="text-sm text-muted-foreground">{t("dashboard.noMovieStatsToday")}</p>
              ) : (
                <div className="space-y-2">
                  {embyTodayStats.movie_top.map((entry, index) => (
                    <div key={entry.key || `${entry.title}-${index}`} className="rounded-xl border border-border/60 bg-background/60 px-4 py-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="truncate text-sm font-semibold">{index + 1}. {entry.title}</p>
