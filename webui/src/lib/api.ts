import type {
  ...
  SigninPublicConfig,
  SigninRenewalResult,
  RegistrationCodeExchangeResult,
  SigninSummary,
  SetupPayload,
  SetupResult,
  SetupStatus,
...
class ApiClient {
  ...
  async getEmbyUrls() {
    return this.request<EmbyLinesInfo>(`/system/emby-urls`);
  }
  ...
  async exchangeRegistrationCode() {
    return this.request<RegistrationCodeExchangeResult>("/signin/exchange-registration-code", { method: "POST" });
  }
  ...
  async searchMoviePilot(query: string) {
    return this.request<unknown>(
      `/moviepilot/search?q=${encodeURIComponent(query)}`,
      {},
      { timeoutMs: 60_000 },
    );
  }

  async addMoviePilotDownload(payload: Record<string, unknown>) {
    return this.request<unknown>("/moviepilot/download", {
      method: "POST",
      body: JSON.stringify(payload),
    });
  }
  ...
  async getEmbyOnline() {
    return this.request<EmbyOnlineInfo>("/emby/online");
  }

  async getEmbyLines() {
    return this.request<EmbyLinesInfo>("/emby/lines");
  }

  async getEmbyTodayStats(limit?: number) {
    const query = new URLSearchParams();
    if (limit) query.set("limit", String(limit));
    return this.request<EmbyTodayStats>(`/emby/stats/today${query.size ? `?${query}` : ""}`);
  }

  async getEmbyRank(period: "daily" | "weekly" | "monthly", type: "movie" | "series", limit?: number) {
    const query = new URLSearchParams({ period, type });
    if (limit) query.set("limit", String(limit));
    return this.request<EmbyRankResponse>(`/emby/stats/rank?${query.toString()}`);
  }

  async getEmbyUserMinutesRank(period: "daily" | "weekly" | "monthly", limit?: number) {
    const query = new URLSearchParams({ period });
    if (limit) query.set("limit", String(limit));
    return this.request<EmbyUserMinutesRankResponse>(`/emby/stats/user-minutes?${query.toString()}`);
  }
  ...
  async batchPromoteWhitelist(selection: number[] | BatchUserSelection) {
    return this.request<BatchUserResult>("/batch/users/promote-whitelist", {
      method: "POST",
      body: JSON.stringify({
        ...this.batchUserSelectionBody(selection),
        confirm: confirmPhrases.batchPromoteWhitelist,
      }),
    });
  }
  ...
  async listEmbyUsers() {
    return this.request<{
      emby_users: Array<{
        ...
        local_user: { uid: number; username: string; telegram_id: number | null; active: boolean; role: number; emby_id?: string | null; emby_bound_at?: number | null } | null;
